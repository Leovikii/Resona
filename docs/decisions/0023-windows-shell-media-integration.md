...
- `MetadataService` 为当前曲目生成有界共享 `Artwork`：按 ADR 0026 选择内嵌或直属目录回退图片，以源字节 SHA-256 维护最多 8 个不同封面的 LRU，相同图片直接复用；源图只解码一次，并归一化为最大 512 px 的 PNG 和 BGRA。归一化 PNG 的稳定指纹供平台消费者比较，PNG 供 SMTC/data URL，BGRA 供 DWM。无封面、过大或损坏时统一使用应用占位图。
