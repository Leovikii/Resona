...
- 普通窗口采用单层材质模型：Mica 是根窗口唯一的大面积背景。Mica 模式下侧栏、窄屏顶部导航、主内容、完整播放器、底栏、压缩窗口标题区和压缩工作区的大面积 `chrome`/`content` 表面保持透明，只用 `hairline` 建立稳定区域边界；不得再用连续半透明黑色或白色表面覆盖根材质。
...
- 每个区域最多增加一层有明确语义的局部实体表面。工具入口、状态带及需要从背景中抬起的紧凑工作单元可使用 `subtle surface`；菜单、Modal、Popover、选中态、错误提示和文字密集交互区继续保持有界实体表面与足够对比度。
- 设置页不是卡片集合。设置分组使用标题、间距和相邻分组的 `hairline` 建立层级，分组本身保持透明、无阴影、无外框和无额外圆角；只有真正独立、可操作的重复项目才使用有界表面。
...
- 单层根材质和少量局部实体表面不会为每行曲目创建合成层，也不会进入播放高频更新路径；相较连续遮罩更容易保持 Mica 可见和视觉层级可预测。

## 验收

- 浏览器使用 `material=mica` 预览验证宽屏 `1080×700`、窄屏 `360×600`、压缩窗口 `720×500` 和桌面歌词；覆盖浅色/深色、零横向溢出、固定底栏/导航几何和控制可达性。浏览器计算样式不能证明 Windows WebView 已透出 Mica，Release 还必须对主内容空白区做原生截图/像素检查，禁止再次出现 solid 回退色。
