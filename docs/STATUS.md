...
- `assets/resona-icon.svg` 已切换为最终单 `R` 设计，并以紧贴字形上下边界的正方形 `viewBox` 消除无效外圈留白，在不拉伸或裁切的前提下提高任务栏和小尺寸图标可读性。Tauri 官方图标生成器已更新 `src-tauri/icons/` 的 Windows、Linux/default、Appx、ICNS、Android 与 iOS 多尺寸资源；`32x32.png`、`128x128.png` 与 `icon.png` 均保留透明播放字腔。0.0.19 仍为 Proposed，安装器、文件关联、Shell/SMTC 身份、托盘关闭语义和签名策略尚待后续决定。
