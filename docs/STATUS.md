...
当前交付状态：0.0.17 已完成。0.0.18 已完成本轮单一当前播放列表模型、外部拖放隔离/命中、默认列表多来源追加、统一添加菜单、播放信息对齐及渲染性能修复，并完成本批选择、批量删除、清空、右键、音质标识、播放中提示、桌面歌词工具栏、播放模式无中断切换、压缩窗口收敛、无回跳定位、歌词点击、统一滚动区和窄屏应用壳；窄屏播放列表入口、直接可见的全量底栏控制和有限范围滚动文本已完成浏览器纠正性验证。普通 Rust 测试 59 通过、8 个真实设备测试默认忽略，其中模式切换连续性测试已单独通过真实设备验证。原生资源管理器拖入、桌面歌词原生 helper 命中、宽窄两套几何的多显示器/DPI 手感与其余 UI 反馈尚未完成项目所有者验收。Windows 图标、安装包、文件关联、应用身份和托盘仍在 Proposed 的 0.0.19；更新器与发行加固仍在 Proposed 的 0.0.20。
