...
`0.1.0` 已发布并完成交付。`0.2.0` 的开发、自动门禁和 Windows 实机功能验收均已完成，
状态为 **Development Complete**，等待进入交付与发行流程。本版本范围是 CUE 逻辑音轨、
按目录回退的共享封面、曲目与目录时长，以及按歌词展示时间同步的桌面长句滚动。
...
- 同名 CUE 的逻辑音轨解析、列表持久化、相对时长/定位播放和逻辑曲目 UI 标识已通过 Windows 实机连续切轨与定位验收。
- 有界目录封面回退沿用共享归一化封面缓存，已通过真实专辑目录、SMTC 与任务栏实机验收。
- 按歌词行区间同步的单向线性长句滚动已通过浏览器自动验证及桌面歌词窗口实机验收。
- 桌面长句动画只在换行、定位、暂停或布局变化时重同步，普通播放轮询不再反复改写动画时钟；播放列表曲目、目录段和整表均显示 Rust 元数据提供的时长，CUE 使用逻辑分轨区间且同一物理文件只解析一次。
- Windows 桌面歌词使用与窗口资源同生命周期的原生 WinEvent 观察器：外部前台窗口变化、虚拟桌面切换和窗口最小化恢复会触发合并后的无激活置顶校正；隐藏时卸载 hook，资源代次会丢弃重建期间的迟到任务。锁定与解锁状态实机验收均未发现新问题。

### 0.2.0 完成门禁

- 前端 42 项测试通过；Rust 111 项通过，9 项依赖真实设备或固定测试工具的用例按设计忽略。
- TypeScript/Vite 生产构建、Rust format、Clippy、许可证生成、工作流 lint 和差异检查通过。
- Windows Release 测试 EXE 构建成功；CUE、封面回退、列表时长、桌面歌词滚动与置顶恢复已完成人工交互验收。
...
- Windows 的 topmost 是共享层级而非绝对最高层；其他持续争夺置顶的窗口、独占全屏和安全桌面不能视为 Resona 可无条件覆盖的普通窗口。0.2.0 已补齐普通前台切换、虚拟桌面和最小化恢复后的无激活置顶校正。
...
- 0.2.0 开发计划已关闭，尚未标记为已发布；后续操作进入受保护的交付与发行流程。
