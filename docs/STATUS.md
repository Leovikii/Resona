...
- 单字母图标使用紫色哥特体 `R` 主体、黑色错位投影和透明播放字腔；完整 `Resona` 字标复用完全相同的 `R` 路径、紫色主体与黑色投影，后续字母通过独立光学位移收紧 `R-e` 间距。中心播放字腔已扩大，后续按原始字体的粗细对比将主干由 `48` 收至 `42` 个设计单位、上方斜笔的视觉宽度由约 `24` 增至 `30`，以约 `1.4:1` 的视觉重量均衡主干、斜笔和腿部，而非改成失去哥特特征的机械等宽。紫色前景保证透明图标在黑色任务栏/状态栏上仍可读，黑色投影在浅色背景上提供立体层次。SVG 已通过 XML 解析，并在亮暗背景、完整字标及 `24/32/48px` 图标尺寸下完成浏览器实渲染检查。
