...
0.0.18 播放列表交互与 UI 冻结：计划已建立，状态为 Proposed，等待项目所有者确认；尚未开始实现。
...
当前交付状态：0.0.17 纠正性实现已完成并通过自动验收。新反馈确认外部文件仍无法加入用户播放列表，完整播放器详情文本排版及其他 UI 细节仍需收口；这些项目作为 0.1.0 发行阻塞/视觉门禁进入 Proposed 的 0.0.18。Windows 图标、安装包、文件关联、应用身份和托盘进入 Proposed 的 0.0.19；更新器与发行加固进入 Proposed 的 0.0.20。
...
- [ ] 确认 [0.0.18 播放列表交互与 UI 冻结计划](plans/0.0.18.md)：Proposed
...
1. 项目所有者审阅并确认 0.0.18-0.1.0 的发行收敛顺序；未经确认不启动新阶段开发。
2. 在 0.0.18 开发前一次性补充剩余 UI 微调意见；随后修复外部文件拖入播放列表，收口播放信息排版并冻结功能 UI。
3. 0.0.18 完成后讨论并确定最终图标、NSIS/MSI、托盘/关闭语义和签名策略，再接受 0.0.19；更新托管和 updater 密钥策略在 0.0.20 开始前确认。
...
0.0.17 已完成。后续使用发现外部文件不能加入用户播放列表，该问题阻塞 0.1.0，但不回改 0.0.17 完成事实，纳入 0.0.18 修复。Windows UI Automation 无法访问 WebView2 内的 React 控件，因此原生外部拖入与命中手感必须保留项目所有者实机验收；路径分类、插入契约和事务边界应由自动测试覆盖。
...
| Windows 资源管理器外部文件拖入用户播放列表 | 0.0.18 | 已确认当前不可用；既定契约不变，待修复和实机验收 |
| 完整播放器详情长文本与最终 UI 一致性 | 0.0.18 | 功能可用；待集中反馈、视觉回归和 UI 冻结 |
