...
### 2026-07-22 0.0.19 品牌图标候选设计

- 根据项目所有者提供的哥特体参考图，在根目录 `assets/` 新增透明背景的 `resona-r-mark.svg` 与横向 `resona-gothic-wordmark.svg`。两者均为项目内自绘路径，不依赖外部字体文件或新增字体许可证。
- 单字母图标使用黑色哥特体 `R` 主体、紫色错位结构层和紫色播放三角字腔；完整 `Resona` 字标复用相同的折角骨架、黑色主体与紫色结构层。SVG 已通过 XML 解析和浏览器实渲染检查。
- 当前文件仅是供项目所有者比较和修订的品牌候选；既有 `assets/resona-icon.svg` 与发布图标未替换，0.0.19 仍为 Proposed，待最终图标、安装器、托盘和签名策略共同确认后再正式接受。
...
当前交付状态：0.0.18 已完成。已完成本轮单一当前播放列表模型、外部拖放隔离/命中、默认列表多来源追加与按位置插入、统一添加菜单、播放信息对齐及渲染性能修复，并完成本批选择、批量删除、清空、右键、音质标识、播放中提示、桌面歌词工具栏、播放模式无中断切换、压缩窗口收敛、压缩控制偏好持久化、无回跳定位、歌词点击、统一滚动区、窄屏应用壳、原生材质收口和主题色预设；窄屏播放列表入口、直接可见的全量底栏控制和有限范围滚动文本已完成浏览器纠正性验证。普通 Rust 测试 67 通过、8 个真实设备测试默认忽略，其中模式切换连续性测试已单独通过真实设备验证。原生资源管理器向默认列表按位置拖入、桌面歌词原生 helper 命中、宽窄两套几何的多显示器/DPI 手感保留为 0.1.0 实机发行门禁，不再重开 0.0.18 功能范围。Windows 图标、安装包、文件关联、应用身份和托盘进入 Proposed 的 0.0.19；更新器与发行加固仍在 Proposed 的 0.0.20。
...
- [ ] 接受并开始 [0.0.19 Windows 分发与应用身份计划](plans/0.0.19.md)：Proposed，待项目所有者确认图标、安装包、托盘和身份策略
- [ ] 确认 [0.0.20 更新器与发行加固计划](plans/0.0.20.md)：Proposed
- [ ] 确认 [0.1.0 Windows 首个正式版发行门禁](plans/0.1.0.md)：Proposed

## 下一步

1. 审阅并确认 Proposed 的 0.0.19 范围：最终图标、NSIS/MSI 选择、文件关联、SMTC/Shell 身份、托盘/关闭语义和签名策略；确认前不得提前实现。
2. 将宽屏/窄屏几何、最大化、多显示器/DPI、资源管理器拖入、当前列表播放中编辑和桌面歌词 helper 保留为 0.1.0 Windows 实机发行门禁；这些验证不再改变已冻结的 0.0.18 页面结构。
3. 0.0.20 开始前单独确认更新托管、签名和 updater 密钥策略；本阶段继续不维护 GitHub Actions。
