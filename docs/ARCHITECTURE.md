...
文件夹导入由 Rust 文件系统边界递归解析为按路径稳定排序的音频项目，并为每项携带可选导入根目录；扫描会规范化、去重且不跟随符号链接、junction 或 reparse point。用户列表把该根目录上下文随项目持久化，默认列表只在会话内保存。React 依据项目顺序和根目录生成可折叠目录段，并按列表维护会话级展开状态；目录节点不是可播放项目，折叠、展开或同一路径因手工排序形成多个连续段时都不得重排底层项目或播放序列。底栏定位动作读取 Rust 权威活动列表与当前路径，只在用户明确触发时展开祖先目录、滚动并聚焦曲目行。
...
播放列表可见行通过 `MetadataService` 的独立轻量摘要读取标题与音轨号；该路径关闭封面和音频属性解析，批量请求有上限并使用小型 LRU，不占用当前曲目封面缓存。文件信息弹窗只在用户明确打开时读取完整标签与音频属性。文件管理器定位经 Tauri opener 的跨平台 reveal 能力封装在 bridge/command 边界，不向 UI 暴露 Explorer、Shell 或 Linux 文件管理器细节。
...
歌词解析器只处理 LRC、SRT 和文本，返回平台无关的时间片/文本模型。桌面歌词窗口由 Rust 服务管理位置、尺寸、置顶、穿透和 helper；Windows adapter 在显示、锁定、解锁、失焦、恢复和 DPI 变化时以原生事件校正 topmost 与 helper 几何，不使用定时置顶。主窗口关闭、托盘隐藏与完整退出由 `ApplicationLifecycleService` 协调。
...
SQLite schema v6 只保存用户播放列表、列表项目（含可空导入根目录上下文）和应用状态。旧 `recent_plays`、`managed_folders` 和 `media_records` 会迁移删除；根目录上下文不建立媒体库或文件夹监视。偏好由 Rust 类型化存储；目录展开等窗口临时 UI 状态只留在前端会话。
