...
- 底部播放器稳定挂载。完整播放器只由左侧封面按钮展开/收起，封面 hover/focus 显示对应动作图标；右侧不保留重复入口。展开或退出时不能重新创建播放控制状态或改变队列；Mica 下完整播放器复用主内容的单层 content fill，自身保持透明，底栏与侧栏直接透出同一根 Mica，不以额外 tonal block、整宽 hairline 或白边切割页面；solid 回退使用对应的不透明 content/player token。
...
- Mica 是根窗口背景能力，不是第二套主题或组件库。标题栏、侧栏、窄屏顶部导航和底栏组成连续的纯 Mica 外壳；主内容只允许一层低 alpha content fill，完整播放器在其中保持透明。播放列表标题与普通曲目行不创建卡片层，只在交互和播放状态时上色。工具入口等真正独立的工作单元可使用一层 `subtle surface`，菜单、文字密集交互区、选中态和错误提示保持实体表面；禁止连续遮罩、全局 Acrylic、`backdrop-filter`、逐行模糊层或用整高/整宽 hairline 切割应用壳。设置分组使用标题和节间距，不使用分组外框、阴影或贯穿页面的分隔线。
- `UiPreferences` 只包含 `colorScheme`、`accentColor` 和 `locale` 等展示偏好。0.0.5 可用 localStorage 持久化，字段保持框架无关，以后可迁移到统一设置服务。
- `accentColor` 只允许 Mantine 原生 `violet`、`blue`、`green`、`pink`、`yellow`；默认 `violet`。主题 provider 统一配置浅色 `primaryShade: 7`、暗色 `primaryShade: 5`、`autoContrast` 和 `luminanceThreshold: 0.3`，feature 不复制色值或自行计算前景色。
...
- `FfmpegDependencyService` 是可选转换依赖的权威来源：只在用户明确操作后下载固定归档，执行流式进度、取消、归档/双二进制 SHA-256、安全解压和同卷原子启用，最终存入 Local AppData；安装包不包含 FFmpeg/ffprobe。
- `CompressionService` 是 WAV 到 FLAC 任务状态的权威来源；WebView 只提交路径、档位和经确认的删除意图并轮询类型化快照，不拼接 FFmpeg 参数。固定运行时依赖、临时文件、ffprobe 验证、原子提交和源文件删除顺序全部在 Rust 内终止。
