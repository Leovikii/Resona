...
应用服务按当前产品需求和变化边界拆分：播放协调、队列、播放列表、最近历史、按需元数据、转换、歌词和设置相互独立。0.1.0 不建立受管理文件夹或媒体库索引，也不建立没有明确需求的动态插件 ABI 或皮肤引擎。

## 前端边界

### 状态归属

- Rust：播放状态、队列、临时默认列表、持久化用户列表、最近历史、转换任务和持久设置。
...
- 底部播放器稳定挂载。完整播放器只由左侧封面按钮展开/收起，封面 hover/focus 显示对应动作图标；右侧不保留重复入口。展开或退出时不能重新创建播放控制状态或改变队列；完整播放器和底栏使用同一不透明主题表面，展开时不保留视觉分割白边。
- 底栏右侧工具使用固定等距槽位，顺序为播放模式、桌面歌词、锁定预留槽、音量和播放队列；播放队列固定在最右。桌面歌词锁定状态或音量交互不得改变底栏网格，音量使用固定按钮和按需弹出的滑杆。
...
- `PlaybackQueue` 表示当前会话的有序播放序列，只有一个权威实例；它由临时默认列表、显式文件或已保存用户列表投影生成。
- `DefaultPlaylist` 表示应用会话内由单文件目录上下文生成的临时路径集合，不写入 SQLite，应用重启后为空。
- `UserPlaylist` 表示可保存的曲目引用集合，不保存解码器、音频数据或媒体索引行；用户列表与队列是不同对象。
- `RecentPlay` 是有限长度的播放事件历史，采用独立保留策略，不能作为队列状态的替代品，也不构成媒体库。
- 文件路径、显示名和队列项 ID 通过领域契约传递；Rodio、CPAL、Symphonia、SQLite 和 Tauri 类型不能泄漏到前端或领域模型。
- 列表插入、追加和排序是集合服务的操作：拖放以目标项 ID 或末尾位置表达，按钮追加明确使用末尾位置；集合服务完成顺序变更后再生成新的播放队列快照，UI 不直接修改音频 actor。
- 0.0.12 中，持久化播放列表项目的 `append`、`insert_at`、`remove` 和 `move` 由 persistence adapter 在事务内维护稠密位置；这些操作不会隐式改变当前播放队列。只有明确的“载入”“追加”或“播放项目”命令才把用户列表投影到 `PlaybackQueue`。
- 单文件打开由统一 `open_media_context` 服务枚举直属支持音频并生成规范化路径快照。快照与当前默认列表相同且目标存在时只切换目标；变化时才原子替换默认列表和队列来源。
- 0.1.0 不保存受管理文件夹、媒体记录或封面索引。`metadata.rs` 只为当前曲目或明确列表展示按需解析，第三方类型仍在 adapter 内终止。

队列来源编码为 `DefaultPlaylist`、`ExplicitFiles`、`UserPlaylist` 等有限枚举，并保留来源 revision。初次进程参数、Tauri single-instance 参数和应用内打开必须映射到同一 typed request，不能在平台回调中直接操作 Rodio。
